...
{{
  ...
  "extra_note": "附加备注（如工厂代发等）"
    ...
    system_prompt = """你是一个极速订单提取助手。输出格式：JSON。
字段提取规则：
1. receiver: 姓名。必须保留原样括号（如[1783]）。
2. phone: 手机号（保留原样）。
3. address: 详细地址。
4. products: [ {name:'名', qty:数量, price:单价} ]。注意货品名如有 *50 这种后缀，不要误切。
5. payment_status: '未付'即为'未付', 看到具体金额或'已付'则为'已付'。

【重要】
- 名字或地址后方的 [1783] 这种内容必须原样保留在对应字段中。
- 不要花费时间猜测业务员或复杂的收款逻辑，仅提取字面信息。
- 直接返回 JSON 字符串，禁止输出解释。"""
